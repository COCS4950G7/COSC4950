                ...
                while not dictionary.isEof() and not self.found_solution.value:  # Keep looping while it is not eof
                    ...
                    if self.single_user_mode:
                        ...
                        new_chunk = manager.Value(dict, {'params': chunk.params,
                                                         ...
                                                         'timestamp': time.time(),
                                                         'halt': False})
                        job_queue.put(new_chunk)  # put next chunk on the job queue. queue is blocking by default,
                                                # so will just wait until it is no longer full before adding another.
